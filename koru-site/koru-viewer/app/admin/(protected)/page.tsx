...
export default function AdminDashboardPage() {
  ...
  if (loading) {
    return (
      <div role="status" aria-label="Carregando dashboard..." className="w-full flex items-center justify-center py-32">
        <div aria-hidden="true" className="w-6 h-6 rounded-full border-2 animate-spin" style={{ borderColor: "var(--border)", borderTopColor: "var(--foreground)" }} />
      </div>
    )
  }
...
function StatCard({ label, value, sub, icon, color, href }: { label: string; value: string; sub: string; icon: React.ReactNode; color: string; href?: string }) {
  const content = (
    <>
      <div aria-hidden="true" className="shrink-0" style={{ color: "var(--muted-foreground)" }}>{icon}</div>
      ...
      {href && (
        <svg aria-hidden="true" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 opacity-0 group-hover:opacity-50 transition-opacity" style={{ color: "var(--muted-foreground)" }}>
